...
function findSources(directory, limit) {
  /** @type {string[]} */
  const found = []
  const visit = current => {
 ...
 * @returns {PluginScan}
 */
export function scanPluginSource(directory) {
  /** @type {Set<string>} */
  ...
  const signals = []
  const files = findSources(directory, MAX_SOURCE_FILES)
 ...
 */
export function judgePlugin(plugin, environment, runtime = {}) {
  const scan = scanPluginSource(plugin.directory)
 ...
 * @param {{ environment: Environment, target?: string, runtime?: { probe?: (name: string) => boolean | null } }} input
 * @returns {{ environment: Environment, plugins: PluginVerdict[], resolvedTarget: string | null }}
 */
export function audit(input) {
  const { environment, target, runtime } = input
  ...
  if (typeof target === 'string' && target.trim() !== '') {
    ...
    return {
      environment,
      plugins: [judgePlugin(plugin, environment, runtime)],
      resolvedTarget: plugin.directory,
    }
  }

  const plugins = listInstalledPlugins(profileDir).map(plugin => judgePlugin(plugin, environment, runtime))
