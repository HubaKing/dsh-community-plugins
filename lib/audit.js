 ...
 * @param {Environment} environment
 * @param {PluginVerdict[]} plugins
 * @returns {ProfileIssue[]}
 */
export function checkProfileIntegrity(environment, plugins) {
  ...
  for (const name of bundles) {
    if (installed.has(name) || environment.officialPackages.has(name)) continue
    issues.push({
      level: 'blocker',
      detail: `dsh.profile.bundles lists "${name}", which is not installed in this profile and is not an official `
        + 'package: the launcher cannot resolve the bundle, so the profile does not boot',
    })
  }
  return issues
}
...
export function audit(input) {
  ...
  if (typeof target === 'string' && target.trim() !== '') {
    ...
    return {
      ...
      profileIssues: checkProfileIntegrity(environment, plugins),
    }
  }
