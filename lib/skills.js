/**
 * Registers the packaged `skills/<name>/SKILL.md` bundles on `ctx.skills`, so
 * every session on this deployment sees the community-plugin guide.
 *
 * Frontmatter is parsed with the same YAML library the official skill providers
 * use (`yaml`), so these bundles follow the official SKILL.md frontmatter format
 * exactly.
 * @module dsh-community-plugins/skills
 */
 ...
 * Throws at mount time when no bundle is present, rather than serving an empty
 * catalog later.
